...
module.exports = new Command({
	name: "playlist",
    aliases: ['pl'],
	description: "The song specified playlist",
	permission: "SEND_MESSAGES",
    options: [
        { description: 'URL Playlist', name: 'playlist', required: true, type: 3 }
    ],
	async run(message, args, client, slash) {
        ...
        if(searchResult.playlist) {
            ...
            const reply = { embeds: [{
                description: `This command does not support play.\nUse **${client.prefix}play** instead please disconect sayu.`, color: 0xb84e44,
            }], ephemeral: true, failIfNotExists: false };
            slash ? message.editReply(reply) : message.reply(reply);
            return;
        }
        slash ? message.editReply(reply) : message.reply(reply);
