...
module.exports = new Event("interactionCreate", async (client, interaction) => {
    ...
    if (interaction.componentType === ComponentType.Button && interaction.customId.includes("buttoncontrol")) {
        ...
        switch(interaction.customId){
            case "buttoncontrol_play":
                ...
                break;
            case "buttoncontrol_stop":
                embed.setDescription(`👋 The music playing on this server has been turned off, see you next time.`);
    ...
    if (interaction.componentType === ComponentType.SelectMenu && interaction.customId === "playactivity") {
        if(interaction.member.voice.channel) {
            try {
                await fetch(`https://discord.com/api/v8/channels/${interaction.member.voice.channel.id}/invites`, {
                  ...
                  .then((res) => res.json())
                  .then((invite) => {
                    if (invite.error || !invite.code || Number(invite.code) === 60) {
